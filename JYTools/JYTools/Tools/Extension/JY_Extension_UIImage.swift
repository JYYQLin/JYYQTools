...
extension UIImage {
    ...
    public static func yq_generate_image(color: UIColor, imageSize: CGSize, cornerRadius: CGFloat = 0, roundingCorners: UIRectCorner = .allCorners) -> UIImage? {
        ...
        let localImage = JY_FileManager_Tool.yq_get_image_from_local(imageName: imageName, directory: .caches)
        ...
        DispatchQueue(label: "JY_Extension_UIImage_SaveImage").async {
            JY_FileManager_Tool.yq_save_image_to_local(image: image, imageName: imageName, directory: .caches)
        }
